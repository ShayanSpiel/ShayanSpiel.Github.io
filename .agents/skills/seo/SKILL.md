...
- Complete analytics implementation (see `.agents/skills/analytics/SKILL.md`)
...
10. `.agents/skills/analytics/SKILL.md`
...
| About `/about/` | BreadcrumbList | `src/pages/about.astro` |
...
The SEO skill may check that analytics is present (GA4, PostHog, Search Console verification) but must defer all implementation decisions to `.agents/skills/analytics/SKILL.md`.
...
Do not edit `src/pages/waitlist.astro` or `src/components/showcase/*`. Their analytics comes from BaseLayout (global) — do not add inline analytics or page-specific schema inside showcase components. Do not add hardcoded English to showcase components; route through `t(locale, "waitlist.*")`.
