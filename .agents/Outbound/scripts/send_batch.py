...
def main() -> None:
    ...
    for i, e in enumerate(emails):
        c = by_id[e["lead_id"]]
        feat = e.get("features", {})
        provider = forced_provider or providers.pick_provider(log)
        ...
        if result.get("error") or not str(result.get("id") or "").strip():
            ...
            if ("daily_quota_exceeded" in err_msg or "monthly_quota_exceeded" in err_msg
                ...
                print("      ❌ Provider quota exceeded (429). Stopping the batch — do not "
                      "send again until the quota resets (UTC midnight / renewal).")
                break
