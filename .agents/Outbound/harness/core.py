...
def cycle(batch_id: str, apply: bool = False) -> dict:
    ...
    # 1. OBSERVE — goals status + data problems
    goals = reflect.goals_status(totals, st["meta"])
    problems = reflect.data_problems(totals)
    breaches = reflect.guardrail_breaches(totals, st["meta"])

    # 1b. BOUNCE LEVER — evidence-driven queue depth. A window breach means
    # the risky tier is bouncing again: only verified leads send until the
    # 48h window decays; when it clears, the queue deepens automatically.
    bounce_breach = any(b["name"] == "bounce rate" for b in breaches)
    ...
    # 2. DIAGNOSE — weakest link
    filters = state.get_variable(st, "cohort_filters", {}) or {}
    skip_unverified = bool(filters.get("skip_unverified"))
    weak = reflect.weakest_link(totals, st["meta"], sent_total, skip_unverified)
