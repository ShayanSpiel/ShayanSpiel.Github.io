...
Sending rotation (`Outreach/workflows/email/providers.py`) picks the enabled
...
2. Verify the engine sees it: `python3 -m Outreach goal` and
   `python3 -m Outreach status` (the report's Providers section lists the
   configured rotation; the campaign's daily cap reflects the enabled caps).
3. There is no daemon to restart — the manual cadence reads `.env` on every
   `once` invocation.
