"""The engine-agnostic GOAL -> OBSERVE -> DECIDE -> ACT -> EVALUATE runner."""
...
from .models import EngineContext, Goal, GoalStatus, RunStatus, Stage
from .registry import engines as installed_engines
...
class Runtime:
    def __init__(self, db_path: str | Path, registry: dict | None = None):
        ...
        self.registry = registry or installed_engines()
        for engine in self.registry.values():
            self.store.register_engine_version(engine.id, engine.version, status="deployed")

    def create_goal(self, **values) -> dict:
        if values["engine_id"] not in self.registry:
            raise KeyError(f"engine '{values['engine_id']}' is not installed")
        if values.get("deadline"):
            _timestamp(values["deadline"])
        values.setdefault("engine_version", self.registry[values["engine_id"]].version)
    ...
    def _advance(self, goal_id: str, holder: str) -> dict:
        ...
        for _ in range(8):
            ...
            engine = self.registry[row["engine_id"]]
            goal = Goal(id=row["id"], name=row["name"], engine_id=row["engine_id"],
            ...
            context = EngineContext(
                ...
                memory=self.store.memories(goal.engine_id, goal.id),
            ...
            result = self._call(engine, Stage(cycle["stage"]), context)
            ...
            if current["sequence"] != start_sequence:
                return self.status(goal_id)
        raise RuntimeError("engine exceeded the eight-transition safety limit")

    @staticmethod
    def _call(engine, stage, ctx):
        data = ctx.cycle.get("data") or {}
        if stage is Stage.OBSERVE:
            return engine.observe(ctx)
        if stage is Stage.DECIDE:
            return engine.decide(ctx, data.get("observation") or {})
        if stage is Stage.ACT:
            return engine.act(ctx, data.get("decision") or {})
        return engine.evaluate(ctx, data.get("action_result") or {})
    ...
    def _notification_payload(self, goal, cycle, result, next_stage):
        ...
        return {
            ...
            "run": {"id": cycle["id"], "sequence": cycle["sequence"],
                    "engine_id": goal.engine_id, "engine_version": self.registry[goal.engine_id].version},
    ...
    def _create_child(self, parent_goal_id: str, parent_run_id: str, spec: dict) -> dict:
        required = ("name", "engine_id", "metric", "operator", "target")
        missing = [key for key in required if key not in spec]
        if missing:
            raise ValueError(f"child goal spec missing: {', '.join(missing)}")
        return self.create_goal(
            name=spec["name"], engine_id=spec["engine_id"], metric=spec["metric"],
    ...
    def complete_change(self, task_id: str, *, passed: bool, result: dict,
        ...
        if passed:
            self.store.register_engine_version(task["engine_id"], task["target_version"],
