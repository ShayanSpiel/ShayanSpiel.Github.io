...
2. **Copy:** Improvised hook "Hi I'm Shayan, recept capture and blah blah" — meaningless. Correct method is `.agents/skills/company/copywriting` + `.agents/company/strategy/voice.md` + `icp.md`/`positioning.md` + `focus.md` → simulation → human_reality → discovery → draft. Must be pain-first (40 PDFs at 9am, copy-paste vendor/date/VAT, switch Outlook→PDF→ERP, remember GL codes, hunt PO), not founder intro.
...
- Read: `icp.md` #5, `positioning.md`, `voice.md`, `focus.md`, `copywriting/SKILL.md`, `design/templates/video/narration.json` (tone_contract: masculine ALWAYS, no music, measured durations)
