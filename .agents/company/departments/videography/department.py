...
class VideographyDepartment(EvidenceDepartment, Department):
    ...
    workflows = (
        WorkflowSpec(
            ...
            (),
            graph=(
                WorkflowStep('author_scenario', 'employee', 'videography-specialist', produces=('demo_scenario',), requires=(), skill_ids=('videography',), connection_ids=()),
                WorkflowStep('record_render', 'employee', 'videography-operator', produces=('showcase_video',), requires=('demo_scenario',), skill_ids=('videography',), connection_ids=()),
            ),
        ),
    )
    goal_schema = {
