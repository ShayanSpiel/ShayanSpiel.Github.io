...
TEMPLATE_ROOT = ROOT / ".agents/company/departments/design/templates"
REGISTRY_PATH = TEMPLATE_ROOT / "registry.json"
BOXICONS_CSS = ROOT / "node_modules/boxicons/css/boxicons.min.css"
...
def render_context_tokens():
    ...
    names = set()
    for css in (ROOT / "src/styles/tokens").glob("*.css"):
        names.update(re.findall(r"--[a-zA-Z0-9-]+", css.read_text()))
    for css in (
        ROOT / ".agents/company/departments/design/system/production.css",
