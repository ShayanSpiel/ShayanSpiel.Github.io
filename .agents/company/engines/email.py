"""Compatibility email engine over the company-native Outbound Department."""
...
def outbound_context(dry=False, goal_config=None):
    from ..departments.outbound.context import build_context
...
class EmailEngine(Engine):
    ...
    def act(self, ctx, decision):
        ...
        from ..departments.outbound import execution as outbound_execution
        ...
        if mode == "live" and ctx.goal.metric in {"reply_rate", "positive_reply_rate"}:
            ...
            if capture == "resend_inbound":
                from ..departments.outbound.workflows.email import config as email_config, providers
        ...
        evidence = []
        if mode == "live":
            from ..departments.outbound.workflows.email import outbound
    ...
    def _evaluate_test_inbox(self, ctx, action_result):
        ...
        if failed or len(sent) != len(action_result.get("recipients", [])):
            ...
            proposal = {"engine_id": "email", "from_version": self.version,
                        "target_version": ctx.goal.config.get("system_improvement_target_version", "2.0.1-compat"),
                        "problem": reason,
                        "allowed_files": [".agents/company/engines/email.py", ".agents/company/departments/outbound/workflows/email/providers.py"],
                        "acceptance_tests": [
...
def _observe_test_provider(ctx, action_result):
    ...
    from ..departments.outbound.workflows.email import analytics, providers
