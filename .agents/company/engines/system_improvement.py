...
class SystemImprovementEngine(Engine):
    id = "system-improvement"
    version = "1.1.0"
    description = "Coordinates bounded engine repairs or new-engine builds with approval, acceptance tests, and version evidence."
    goal_schema = {
        ...
        "config": {
            ...
            "change_kind": {"enum": ["repair", "create_engine"]},
            "engine_spec": {"type": "object", "required_when": {"change_kind": "create_engine"}},
        },
    }
    ...
    def decide(self, ctx, observation):
        ...
        required = ("engine_id", "from_version", "target_version", "problem",
                    "allowed_files", "acceptance_tests")
        missing = [key for key in required if not config.get(key)]
        change_kind = config.get("change_kind", "repair")
        if change_kind not in ("repair", "create_engine"):
            missing.append("change_kind(repair|create_engine)")
        if change_kind == "create_engine" and not config.get("engine_spec"):
            missing.append("engine_spec")
        if missing:
    ...
    def act(self, ctx, decision):
        ...
        if action == "create_change_task":
            ...
            task = ctx.create_change_task({
                "engine_id": config["engine_id"], "from_version": config["from_version"],
                ...
                "change_kind": config.get("change_kind", "repair"),
                "specification": config.get("engine_spec", {}),
            })
    ...
    def evaluate(self, ctx, action_result):
        ...
        if passed:
            return StageResult("goal_check", metrics, RunStatus.IDLE, goal_status=GoalStatus.ACHIEVED,
                               evaluation=evaluation, message="Engine change validated and versioned")
        return StageResult("goal_check", metrics, RunStatus.BLOCKED, Stage.EVALUATE,
                           evaluation=evaluation, message="Engine change failed acceptance")
