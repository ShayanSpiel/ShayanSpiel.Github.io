"""Department package shape: serializable Lego blocks Director can install."""
...
def package_spec(department: Department) -> dict[str, Any]:
    """Stable installable description of one Department Lego package."""
...
def validate_package(department: Department) -> list[str]:
    ...
    defects = []
    dept_id = department.department_id or department.id
    if not dept_id:
